...
int main(int argc, char **argv) // pot_type max_l name (N (diff_precision))
{
    // input
    int pot_type = atoi(argv[1]), max_l = atoi(argv[2]); // pot_type 0: well, 1: harmo, 2: morse
    std::string path_name = "results/" + (std::string)argv[3] + "/";
    if (argc >= 5)
        N = atoi(argv[4]);
    if (argc >= 6)
        diff_precision = atoi(argv[5]);
    ...
    for (int l = 0; l <= max_l; l++)
    {
        ...
        out_vecs << std::setprecision(6) << std::scientific << es.eigenvectors() << std::endl;
        out_vals.close();
        out_vecs.close();
    }
    clock_t end = clock();
